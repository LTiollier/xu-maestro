...
- **Plusieurs agents `working` simultanément — seul le premier passe en erreur** — `Object.entries(agents).find(...)` retourne la première entrée `working` seulement. Un workflow parallèle hypothétique avec deux agents concurrents ne mettrait qu'un seul en erreur. Non applicable au modèle d'exécution séquentiel actuel de RunService ; à revisiter si l'exécution parallèle est introduite. [useSSEListener.ts:97]
- **`onerror` peut se déclencher avant `onopen`** — Si le réseau est indisponible dès le début, `onerror` se déclenche avant que `onopen` soit appelé, avec `runStore.status === 'running'` déjà posé par LaunchBar. Le guard `status !== 'running'` laisse passer ce cas, ce qui est correct (setRunError), mais l'ES n'a jamais été `connected`. Cosmétique uniquement (`connectionStatus` passe de `idle` à `error` sans passer par `connected`). Acceptable MVP. [useSSEListener.ts:88]

## Deferred from: log-sse-streaming review (2026-04-15)

- **UX silencieuse si le stream log-stream meurt (onerror→close)** — Quand l'EventSource reçoit une erreur réseau, le log s'arrête sans feedback utilisateur. Pas de bouton Retry, pas d'état d'erreur affiché dans RunSidebar. Intentionnel pour éviter la reconnexion infinie (cf. spec), mais la UX est muette. Amélioration : afficher un badge "stream interrompu" avec bouton Retry manuel. [RunSidebar.tsx]
- **resolveRunPath race avec checkpoint.json partiellement écrit** — Si `json_decode` lit un checkpoint en cours d'écriture (`File::put` non atomique), le `catch` saute le répertoire et retourne null, causant un 404 spurieux. Pré-existant dans RunController@log. Correction : écriture atomique (tmp + rename) dans ArtifactService ou CheckpointService. [LogSseController.php:resolveRunPath]
- **resolveRunPath non recaché après scan FS** — Quand le cache miss force un scan de tous les répertoires runs, le chemin trouvé n'est pas réenregistré en cache. Les connexions concurrentes pour le même runId font chacune un scan O(n). Pré-existant. Correction : `cache()->put("run:{$id}:path", $dir, 3600)` avant return. [LogSseController.php:resolveRunPath]
- **file_get_contents lit le fichier entier à chaque tick** — Pour des runs très longs, session.md peut atteindre des centaines de Ko. L'approche fopen/fseek/fread ne lirait que les nouveaux bytes. Performance acceptable au MVP. [LogSseController.php:stream]

## Deferred from: scaffold-onboarding review (2026-04-17)

- **`GeminiDriver::prompt()` fallback `$result->output()` retourne du stream-json brut** — Si Gemini ne produit aucun message/assistant dans le flux JSON, le fallback retourne tout le stdout (lignes JSON brutes). `Yaml::parse()` échoue et retourne 422 avec ce contenu illisible dans `raw_yaml`. Acceptable au MVP (l'utilisateur voit l'erreur et peut corriger). À améliorer en retournant une exception lisible quand `$accumulatedResponse` est vide. [GeminiDriver.php]
- **`str_contains()` fragile pour discriminer le 409 dans `store()`** — La détection de conflit repose sur `str_contains($e->getMessage(), 'already exists')`. Si le message change, le 409 devient un 500. À corriger via une exception dédiée `WorkflowAlreadyExistsException`. [WorkflowController.php]
- **AbortRef partagé entre `handleGenerate` et `handleStore`** — Un seul `abortRef` sert aux deux opérations. Déclencher `handleStore` pendant un `handleGenerate` en vol annule le generate silencieusement (et vice versa). Acceptable via le gating step 1/2, mais à refactoriser en deux refs séparés si les transitions deviennent plus complexes. [WorkflowWizard.tsx]
- **Agent badge regex peut matcher des clés `id:` hors-agents** — La regex `/^\s+-?\s*id:\s*/` utilisée pour l'affichage des badges peut capturer des clés `id:` imbriquées dans `system_prompt` ou d'autres sous-structures. Impact cosmétique uniquement. À corriger via parsing YAML client-side si les badges deviennent critiques. [WorkflowWizard.tsx]
- **`sub-workflow` engine accepté par `validate()` mais absent du prompt Gemini** — Si Gemini hallucine `engine: sub-workflow`, `validate()` dans `generate()` accepte le YAML (sans vérif du fichier), mais `validate()` dans `store()` rejette si le fichier n'existe pas → 422 "Structure invalide" opaque. Clarifier en retirant `sub-workflow` du scope du wizard ou en améliorant le message d'erreur. [YamlService.php:validate]
